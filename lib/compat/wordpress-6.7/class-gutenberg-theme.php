...
/**
 * Gutenberg_Theme class
 * WP_Theme is a final class, so we can't extend it.
 *
 * @see WP_Theme
 */
class Gutenberg_Theme {
	...
	/**
	 * Constructor for Gutenberg_Theme.
	 *
	 * @since 3.4.0
	 *
	 * @global array $Gutenberg_Theme_directories
	 *
	 * @param string        $theme_dir  Directory of the theme within the theme_root.
	 * @param string        $theme_root Theme root.
	 * @param Gutenberg_Theme|null $_child If this theme is a parent theme, the child may be passed for validation purposes.
	 */
	public function __construct( $theme_dir, $theme_root, $_child = null ) {
		global $Gutenberg_Theme_directories;

		// Initialize caching on first run.
		if ( ! isset( self::$persistently_cache ) ) {
			/** This action is documented in wp-includes/theme.php */
			self::$persistently_cache = apply_filters( 'wp_cache_themes_persistently', false, 'Gutenberg_Theme' );
		...
		if ( ! in_array( $theme_root, (array) $Gutenberg_Theme_directories, true )
			&& in_array( dirname( $theme_root ), (array) $Gutenberg_Theme_directories, true )
		) {
		...
		if ( ! is_array( $cache )
			...
			if ( '.' !== $parent_dir
				...
				$this->parent = new Gutenberg_Theme( $this->template, $this->theme_root, $this );
				return;
			}
		}
		...
		if ( $this->template !== $this->stylesheet ) {
			...
			if ( $_child instanceof Gutenberg_Theme && $_child->template === $this->stylesheet ) {
				$_child->parent = null;
			...
			$this->parent = new Gutenberg_Theme( $this->template, isset( $theme_root_template ) ? $theme_root_template : $this->theme_root, $this );
		}
	 ...
	 * @since 3.4.0
	 *
	 * @return Gutenberg_Theme|false Parent theme, or false if the active theme is not a child theme.
	 */
	 ...
	 * for all other URLs returned by Gutenberg_Theme, so we pass it to the public function
	...
	public function get_page_templates( $post = null, $post_type = 'page' ) {
		 ...
		 * @param Gutenberg_Theme     $theme          The theme object.
	 ...
	 * @since 4.4.0
	 *
	 * @return Gutenberg_Theme|false Object, or false if no theme is installed, which would be bad.
	 */
	...
	public function get_block_patterns() {
		...
		foreach ( $files as $file ) {
			...
			$bool_properties = array( 'inserter', 'synced' );
	...
	private function get_pattern_cache() {
		...
		$pattern_data = get_site_transient( 'Gutenberg_Theme_files_patterns-' . $this->cache_hash );
	...
	private function set_pattern_cache( array $patterns ) {
		 ...
		 */
		$cache_expiration = (int) apply_filters( 'Gutenberg_Theme_files_cache_ttl', self::$cache_expiration, 'theme_block_patterns' );
		...
		if ( $cache_expiration <= 0 ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					'<code>Gutenberg_Theme_files_cache_ttl</code>'
				),
				'6.6.0'
			);
		...
		set_site_transient( 'Gutenberg_Theme_files_patterns-' . $this->cache_hash, $pattern_data, $cache_expiration );
	}
	...
	public function delete_pattern_cache() {
		delete_site_transient( 'Gutenberg_Theme_files_patterns-' . $this->cache_hash );
	}
	 ...
	 * @since 3.4.0
	 *
	 * @param Gutenberg_Theme[] $themes Array of theme objects to sort (passed by reference).
	 */
	public static function sort_by_name( &$themes ) {
		if ( str_starts_with( get_user_locale(), 'en_' ) ) {
			uasort( $themes, array( 'Gutenberg_Theme', '_name_sort' ) );
		} else {
			foreach ( $themes as $key => $theme ) {
				$theme->translate_header( 'Name', $theme->headers['Name'] );
			}
			uasort( $themes, array( 'Gutenberg_Theme', '_name_sort_i18n' ) );
		}
	}
	 ...
	 * @since 3.4.0
	 *
	 * @param Gutenberg_Theme $a First theme.
	 * @param Gutenberg_Theme $b Second theme.
